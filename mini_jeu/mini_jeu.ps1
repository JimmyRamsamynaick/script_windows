...
function Show-Menu {
    do{
        ...
        Write-Host '1. Space Invader' -ForegroundColor White
        Write-Host '2. Nombre Mystère' -ForegroundColor White
        Write-Host '0. Revenir en arrière' -ForegroundColor Red
        Write-Host '──────────────────────────────' -ForegroundColor DarkCyan
        Write-Host ''
        $choice=Read-Host 'Votre choix'
        switch($choice){
            '1' { Show-Countdown -Seconds 3 -Text 'Lancement de Space Invader dans'; Show-Spinner -Text 'Chargement de Space Invader' -Seconds 2; Run-Child $script:InvaderPath }
            '2' { Show-Countdown -Seconds 2 -Text 'Ouverture du jeu Nombre Mystère'; Run-Child $script:GuessPath }
